...
%
%one line to calculate the sigmoid() of the product of X and all_theta. 
%X is (m x n), and all_theta is (num_labels x n), 
% so you'll need a transposition to get a result of (m x num_labels)
%one line to return the classifier which has the max value. 
%The size will be (m x 1). 
%Use the "help max" command in your workspace to learn how the max() function 
%returns two values.
...
Z = sigmoid(num_labels * X');
[maxPrediction, I] = max(Z, [], 2);
p = I';
